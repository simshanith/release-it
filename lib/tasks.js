import { bump, runTemplateCommand as run, pushd, copy, npmPublish, popd, mkStageDir } from './shell';
...
export default async function runTasks() {
  ...
  try {
    ...
    const { path: stageDir, cleanup: rmStageDir } = dist.repo ? await mkStageDir(dist.stageDir) : {};
